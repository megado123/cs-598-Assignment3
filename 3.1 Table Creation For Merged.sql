use cs598_Assignment3Merged

IF OBJECT_ID('saleDealership') IS NOT NULL
    DROP TABLE saleDealership;
GO
...
IF OBJECT_ID('vehicleInventoryDetails') IS NOT NULL
    DROP TABLE vehicleInventoryDetails;
GO

IF OBJECT_ID('salesTransactionsActivity') IS NOT NULL
    DROP TABLE salesTransactionsActivity;
GO
...
CREATE TABLE vehicleInventoryDetails (
    inventoryDetailsId int IDENTITY(1,1),
...
CREATE TABLE associate(
  associateId int IDENTITY(1,1),
  associateName varchar (255),
  PRIMARY KEY (associateId),
);

CREATE TABLE saleDealership(
...
CREATE TABLE sale(
   ...
   saleLog int NOT NULL,
   saleDate Date NOT NULL,
   saleDealershipId int NOT NULL,
   saleAssociateId int NOT NULL,
   customerPurchaserId int NOT NULL,
   salesDiscountId int,
   --note varchar (255),
   PRIMARY KEY (saleId),
   FOREIGN KEY (saleDealershipId) REFERENCES saleDealership (dealershipId),
   FOREIGN KEY (saleassociateId) REFERENCES associate (associateId),
...
CREATE TABLE salesTransactionsActivity (
    transactionIdActivity int IDENTITY(1,1),
	price MONEY,
	vehicleInventoryId int,
    PRIMARY KEY (transactionIdActivity),
	FOREIGN KEY (vehicleInventoryId) REFERENCES vehicleInventory(inventoryId),
);
...
CREATE TABLE salesTransactions (
	...
	saleId int,
	salestransactionIdActivity int,
    PRIMARY KEY (transactionsId),
	FOREIGN KEY (salestransactionIdActivity) REFERENCES salesTransactionsActivity(transactionIdActivity),
